...
def frangi_vesselness(
    ...
    percentile = float(config["frangi_structure_percentile"])
    gamma_factor = float(config["frangi_gamma_factor"])
    vesselness = np.zeros_like(image, dtype=np.float32)
    for sigma in config["frangi_sigmas"]:
        sigma = float(sigma)
        smoothed = cv2.GaussianBlur(image, (0, 0), sigmaX=sigma, sigmaY=sigma).astype(np.float32)
        scale = sigma * sigma
        ...
        discriminant = np.sqrt(np.square(dxx - dyy) + 4.0 * np.square(dxy))
        eigenvalue_a = 0.5 * (dxx + dyy - discriminant)
        eigenvalue_b = 0.5 * (dxx + dyy + discriminant)
        swap = np.abs(eigenvalue_a) > np.abs(eigenvalue_b)
        lambda_1 = np.where(swap, eigenvalue_b, eigenvalue_a)
        lambda_2 = np.where(swap, eigenvalue_a, eigenvalue_b)

        blobness = np.abs(lambda_1) / (np.abs(lambda_2) + 1e-6)
        structure = np.sqrt(np.square(lambda_1) + np.square(lambda_2))
        gamma = max(float(np.percentile(structure[fov_mask], percentile)) * gamma_factor, 1e-6)
        response = np.exp(-np.square(blobness) / (2.0 * beta * beta))
        response *= 1.0 - np.exp(-np.square(structure) / (2.0 * gamma * gamma))
        response[lambda_2 <= 0] = 0.0
...
def segment_vessels(
    ...
    green = image[:, :, 1]
    tile_size = config["clahe_tile_grid_size"]
    clahe = cv2.createCLAHE(
        clipLimit=float(config["clahe_clip_limit"]),
        tileGridSize=(int(tile_size[0]), int(tile_size[1])),
    )
    enhanced_green = clahe.apply(green)
    vesselness = frangi_vesselness(enhanced_green, fov_mask, config)
    maximum = max(float(vesselness[fov_mask].max()), 1e-12)
    normalized = np.clip(vesselness * 255.0 / maximum, 0, 255).astype(np.uint8)
    otsu_threshold, _ = cv2.threshold(
        normalized[fov_mask].reshape(-1, 1),
        0,
        255,
        cv2.THRESH_BINARY + cv2.THRESH_OTSU,
    )
    final_threshold = float(otsu_threshold) * float(config["otsu_threshold_factor"])
